...
function pkproject(){
	...
	oProject.table_cols = ['pId','pName','status','minSquare','maxSquare','minPrice','maxPrice','countryId','proviceId','cityId','districtId','fullAddr','prightLimit','handoverYear','handoverMonth','handoverDay','canLoan','totalSquare','totalAmount','predictYearRent','localPricePic','picture1','picture2','picture3','thumbnail','description'];
	
	// 以下4个变量的长度必须一致！！！
	oProject.table_cross_fkey = [['countryId'],['pId']];	
	// 每个查询外边，对应上面的外键组 以及 下面的查询列
	oProject.table_cross_name = ['area','houselayout'];	
	// 查询列必须以对应外键的ID作为第一个，后面是查询的值列，不限个数
	// 2018.12.17 加入二级关联，在关联表中如果还有需要翻译的关联字段，则将这个字段翻译成对象，：{fkey: typeId, table: 'type', cols: [ 'id', 'name' ]}
	oProject.table_cross_cols = [['addrId','name'],
